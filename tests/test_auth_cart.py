...
def test_register_success(client, monkeypatch):
    ...
    # SELECT existing -> None, then INSERT RETURNING -> user dict
...
def test_cart_add_guest_requires_session(client):
    # No session_id and no auth -> should return 400
...
def test_cart_get_no_args_returns_empty(client):
    ...
    # Endpoint returns [] when no session_id and unauthenticated
    assert res.get_json() == []
